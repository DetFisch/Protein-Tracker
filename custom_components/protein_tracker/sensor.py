...
from .naming import base_display_name, tracker_display_name
...
async def async_setup_entry(
    ...
    entities = [ProteinTodaySensor(manager, user_id) for user_id in manager.user_ids()]
...
class ProteinTodaySensor(CoordinatorEntity[ProteinTrackerManager], SensorEntity):
    ...
    async def async_added_to_hass(self) -> None:
        """Ensure stable entity_id format sensor.<configured_id>."""
        await super().async_added_to_hass()
        if self.hass is None:
            return

        desired_entity_id = f"sensor.{self._user_id}"
